...
int exec_bytecode() {
  ...
  while(1)
  {
	unsigned int reader = text [TEXT_POS(pc)];

	if (reader != 0x00){
		//masking opcode
		int opcode = (reader & 0xFC000000 );
		int shift_opcode = (opcode >> 26);
		
		//r type
		int function_r = (reader & 0x3F);
		
		int rs = (reader & 0x3E00000);
		int shift_rs = (rs >> 21);
		//location 
		int rs_register = (registers [shift_rs]);

		int rt = (reader & 0x1F0000);
		int shift_rt = (rt >> 16);
		//location 
		int rt_register = (registers [shift_rt]);

		int rd = (reader & 0xF800);
		int shift_rd = (rd >> 11);
		//location 
		int rd_register = (registers [shift_rd]);
		
		int amt = (reader & 0x7C0);
		int shift_amt = (amt >> 6);
		//location 

		//opcode add
		if (shift_opcode == 0x00 && function_r == 0x20){

			//rd = rs+rt
			rd_register = rs_register + rt_register;
			//update values into register array
			registers [shift_rd] = rd_register;
			pc += 4;
		}

		//opcode sll
		if (shift_opcode == 0x00 && function_r == 0x00){

			//rd=rt<<sa
			rd_register = rt_register << shift_amt;
			//update values into register array
			(registers [shift_rd]) = rd_register;
			pc += 4;
		}

		if (shift_opcode == 0x00 && function_r == 0x02){
		...
		//i type
		int imm = (reader & 0xFFFF);
		
		if (shift_opcode == 0x08)
		{
			
			int sign_imm = (reader & 0x8000);
			int shift_opcode = (opcode >> 15);
			
			if(sign_imm == 0){
				//rt=rs+imm	
				rt_register = rs_register + imm;
			}
			else{

				int flipAdd = (imm ^ 0xFFFF) + 1;
				//rt=rs+imm	
				rt_register = rs_register - flipAdd;
			}
			registers[shift_rt] = rt_register;
			pc += 4;
		}
		
		//opcode andi
		if (shift_opcode == 0x0C)
		{
			registers[shift_rt] = rs_register & imm;
			pc += 4;
		}
		
		//opcode lui
		if (shift_opcode == 0x0F) 
		{
        int imm = (reader & 0xFFFF);
        registers[shift_rt] = imm << 16;
		pc += 4;
      	}

		
		// opcode bne
		if (shift_opcode == 0x05) 
		{
			// (BNE) instruction

			unsigned int imm = (reader & 0xFFFF);

			if (registers[shift_rs] != registers[shift_rt]) {
		...
		// opcode lw
		if (shift_opcode == 0x23) 
		{
			int sign = (imm << 16) >> 16;
			int address = rs_register + sign;
			registers[shift_rt] = ntohl(data[DATA_POS(address)]);
		}

		// opcode sw
		if (shift_opcode == 0x29) 
		{
			int sign = (imm << 16) >> 16;
			int address = rs_register + sign;
			data[DATA_POS(address)] = htonl(registers[shift_rt]);
		}
